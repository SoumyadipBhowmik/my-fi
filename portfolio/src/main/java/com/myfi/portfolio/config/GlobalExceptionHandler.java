...
public class GlobalExceptionHandler implements ServerInterceptor {
    ...
    private <ReqT, RespT> void handleException(
        ...
        Status status = switch (e) {
            case AlreadyExistsException ignored -> Status.ALREADY_EXISTS.withDescription(e.getMessage());
            case InvalidArgumentException ignored -> Status.INVALID_ARGUMENT.withDescription(e.getMessage());
            case NotFoundException ignored -> Status.NOT_FOUND.withDescription(e.getMessage());
            case ServiceNotAvailableException ignored -> Status.UNAVAILABLE.withDescription(e.getMessage());
